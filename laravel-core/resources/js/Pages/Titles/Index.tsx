...
import { Blocks, Calendar, Captions, CheckSquare, ChevronDown, Contact, Edit2, Facebook, Film, Hash, Headphones, Image, KeyRound, Layers, LayoutPanelTop, MessageSquare, MessageSquareText, ScrollText, Search, Trash, Trash2, User } from 'lucide-react';
...
const TitlesIndex: React.FC<PageProps<{ groups: TitlesGroup[], from:number, to:number, total:number }>> = ({ auth, groups, from, to, total, menu }) => {
    ...
    const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const titlesToFilter = activeGroups ? activeGroups : [];
                        ...
                        <table className="table table-report -mt-2">
                            ...
                            {activeGroups && activeGroups.map((group, index) => (
                                <tr key={index} className="intro-x">
                                    ...
                                    <td>
                                        {group.titles.map((group, index)=>(
                                        <div key={index} className="flex items-center">
                                            <Captions className="h-4 w-4 text-gray-500 mr-1" />
                                            {group.title && group.title.length > 20 
