...
import { PageProps, LocationsGroup, Location } from '@/types';
...
const LocationsIndex: React.FC<PageProps<{ groups: LocationsGroup[], from:number, to:number, total:number }>> = ({ auth, groups, from, to, total, menu }) => {
    ...
    const [activeGroups, setActiveGroups] = useState<LocationsGroup[]>(groups)
    ...
    ? activeGroups.reduce((latest, location) => 
        new Date(location.updated_at).getTime() > new Date(latest.updated_at).getTime() ? location : latest
    ) 
    : null;
    ...
    const handleDeleteGroup = async () => {
        ...
        try {
            await groupForm.delete(route('locations.destroy', { id: groupForm.data.group }));
            toast.success('Group of locations has been deleted successfully');
            router.get(route('locations.index'));
        } catch(error) {
            toast.error('Error deleting the group of locations');
    ...
    const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const locationsToFilter = groups ? groups : [];
        const searchTerm = event.target.value.toLowerCase();
        const filteredLocations = locationsToFilter.filter(item => 
            (item.name && item.name.toLowerCase().includes(searchTerm)) ||
            (item.description && item.description.toLowerCase().includes(searchTerm)) ||
            (item.created_by && item.created_by.name.toLowerCase().includes(searchTerm))
        );
        setActiveGroups(filteredLocations)
    };
    return (<>
        ...
        <Webmaster
            user={auth.user}
            menu={menu}
            breadcrumb={<li className="breadcrumb-item active" aria-current="page">Locations</li>}
        >
        <Page title="Locations" header={<></>}>
                ...
                <div className="col-span-12">
                    <div className="intro-y flex flex-col-reverse sm:flex-row items-center">
                        <div className="w-full sm:w-auto relative mr-auto mt-3 sm:mt-0">
                            ...
                            <input type="text" className="form-control w-full sm:w-64 box px-10" placeholder="Search location" onChange={handleSearchChange}/>
                        ...
                        <Link href={route('locations.create')} className="btn btn-primary shadow-md mr-2">Create Group of Locations</Link>
                        </div>
                    </div>
                        ...
                        <table className="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th className="whitespace-nowrap">Locations</th>
                                ...
                                <tr key={index} className="intro-x">
                                    ...
                                    <td>
                                        ...
                                        if(index < 2){
                                            return (
                                                ...
                                                <div key={index} className="flex items-center">
                                                    <LayoutList className="h-4 w-4 text-gray-500 mr-1" />
                                                    {group.locations.length - 2} more communes
                                                </div>)
                                            }
                                            return null;
                                        })}
                                    ...
                                    <td className="table-report__action w-56">
                                        <div className="flex justify-center items-center">
                                            <Link className="flex items-center mr-3" href={route('locations.edit', { group: group.id })}>
