...
const CreateTag: React.FC<PageProps> = ({auth, menu}) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setCreating(true);
        tagsGroupForm.post(route('tags.store'), {
            onSuccess: () => {
                toast.success('Group of tags has been created successfully');
                router.get(route('tags.index'));
            },
            onError: (error) => {
                toast.error('Error creating the group of tags');
    ...
    return (<>
        <Head title="Create a group of tags" />
        <Webmaster
            user={auth.user}
            menu={menu}
            breadcrumb={<>
        ...
        >
        <Page title="Create a group of tags" header={<></>}>
            <Grid title="Groups information">
                ...
                <CustomTextarea title="Description" value={tagsGroupForm.data.description} name='description' description='Enter the description of the group' required={false} handleChange={handleChange} instructions='Not required'/>
