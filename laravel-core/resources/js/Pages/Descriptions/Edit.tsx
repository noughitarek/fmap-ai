import { PageProps, DescriptionsGroup } from "@/types";
...
const EditDescription: React.FC<PageProps<{group: DescriptionsGroup}>> = ({auth, menu, group}) => {
    const [editing, setEditing] = useState(false)
    const descriptionsGroupForm = useForm<DescriptionsGroupForm>({
        name: group.name || '',
        description: group.description || '',
        descriptions: group.string_descriptions || [],
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setEditing(true);
        descriptionsGroupForm.post(route('descriptions.update', {group: group.id}), {
            onSuccess: () => {
                toast.success('Group of descriptions has been updated successfully');
                router.get(route('descriptions.index'));
            },
            onError: (error) => {
                toast.error('Error updating the group of descriptions');
    ...
    return (<>
        <Head title="Edit a group of descriptions" />
        <Webmaster
            user={auth.user}
            menu={menu}
            breadcrumb={<>
        ...
        >
        <Page title="Edit a group of descriptions" header={<></>}>
