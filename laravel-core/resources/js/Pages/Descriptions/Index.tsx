...
import { PageProps, DescriptionsGroup, Description } from '@/types';
...
const DescriptionsIndex: React.FC<PageProps<{ groups: DescriptionsGroup[], from:number, to:number, total:number }>> = ({ auth, groups, from, to, total, menu }) => {
    ...
    const [activeGroups, setActiveGroups] = useState<DescriptionsGroup[]>(groups)
    ...
    ? activeGroups.reduce((latest, description) => 
        new Date(description.updated_at).getTime() > new Date(latest.updated_at).getTime() ? description : latest
    ) 
    : null;
    ...
    const handleDeleteGroup = async () => {
        ...
        try {
            await groupForm.delete(route('descriptions.destroy', { id: groupForm.data.group }));
            toast.success('Group of descriptions has been deleted successfully');
            router.get(route('descriptions.index'));
        } catch(error) {
            toast.error('Error deleting the group of descriptions');
    ...
    const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const descriptionsToFilter = groups ? groups : [];
        const searchTerm = event.target.value.toLowerCase();
        const filteredDescriptions = descriptionsToFilter.filter(item => 
            (item.name && item.name.toLowerCase().includes(searchTerm)) ||
            (item.description && item.description.toLowerCase().includes(searchTerm)) ||
            (item.created_by && item.created_by.name.toLowerCase().includes(searchTerm))
        );
        setActiveGroups(filteredDescriptions)
    };
    return (<>
        <Head title="Descriptions" />
        <Webmaster
            user={auth.user}
            menu={menu}
            breadcrumb={<li className="breadcrumb-item active" aria-current="page">Descriptions</li>}
        >
        <Page title="Descriptions" header={<></>}>
                ...
                <div className="col-span-12">
                    <div className="intro-y flex flex-col-reverse sm:flex-row items-center">
                        <div className="w-full sm:w-auto relative mr-auto mt-3 sm:mt-0">
                            ...
                            <input type="text" className="form-control w-full sm:w-64 box px-10" placeholder="Search description" onChange={handleSearchChange}/>
                        ...
                        <Link href={route('descriptions.create')} className="btn btn-primary shadow-md mr-2">Create Group of Descriptions</Link>
                        </div>
                    </div>
                        ...
                        <table className="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th className="whitespace-nowrap">Descriptions</th>
                                ...
                                <tr key={index} className="intro-x">
                                    ...
                                    <td>
                                        ...
                                        <div key={index} className="flex items-center">
                                            ...
                                            {description.description && description.description.length > 20 
                                    ...
                                    <td className="table-report__action w-56">
                                        <div className="flex justify-center items-center">
                                            <Link className="flex items-center mr-3" href={route('descriptions.edit', { group: group.id })}>
